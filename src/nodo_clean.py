...
class occupancy_grid_class_copy():
    ...
    def _init_(self):  # this is colled "method of the class" ?
        ...
        self.grid = np.zeros(
            (self.n, self.m))  # it's a matrix that will be copied from "/map" topic and reshaped afterwards
        ...
        self.SanitizedMap = OccupancyGrid(
            header=rospy.Header())  # OLD COMMENT# we will use this element as a OccupacyGrid for storing sanification data

        self.AreaToSanitize = GridCells(
            header=rospy.Header())  # this is an object that will be sent to rviz where we will see the area to be sanitized

        self.SanitizedArea = GridCells(
            header=rospy.Header())  # this is an object that will be sent to rviz where we will see the areas that have been sanitized

        self.WallsCells = GridCells(
            header=rospy.Header())  # this is an object that will be sent to rviz where we will see the cell that aproximate the walls

        self.UnexploredCells = GridCells(
            header=rospy.Header())  # this is an object that will be sent to rviz where we will see the cells that werent explored when the map was created

    def callbackFillSanitizazionMap(self,
                                    data):  # the callback take always an additional argument (in this case "data") that represent the object that you will manage
        '''
                ...
                for j in range(-radius, radius + 1):
                    ...
                    if sqrt(i * 2 + j * 2) >= radius and self.roomLimitCheck2(xx_pose + i, yy_pose + j, room):
                        ...
                        if room[
                            ...
                            # You cannot change the values of angles also for the cells that have your same raidus
...
if _name_ == '_main_':  # this line of code is used to execute the code below if it is run from terminal, if this python file is imported from another python code the part below will not be executed
    try:
...
class occupancy_grid_class_copy():
    ...
    def _init_(self):# this is colled "method of the class" ?
        ...
        self.disc_n = 2# discretized lenth in x direction (zero axes) of self.discretized_grid
        self.disc_m = 2# discretized lenth in y direction (one axes) of self.discretized_grid
        self.discretized_grid = np.zeros((self.disc_n, self.disc_m))# it's a discretized matrix of the envirmant that will be obtained from self.grid
    ...
    def callbackFillSanitizazionMap(self, data):# the callback take always an additional argument (in this case "data") that represent the object that you will manage
        ...
        # displays the unique elements of the self.discretized grid where "-1" represents the unexplored region
        # ,"0" represents the areas to be sanitized, "10" represents the walls and "1" represents the areas that have been sanitized or are free
        print("\nElements of self.discretized_grid = {0}".format(np.unique(np.reshape(self.discretized_grid, (np.shape(self.discretized_grid)[0]**2)))))

        # displays data about self.discretized_grid
        print("\nself.disc_n: {0} \tself.disc_m: {1}\tself.discretized_grid: {2}\n".format(self.disc_n, self.disc_m, self.discretized_grid))
        ...
        for q in range(0, self.disc_n):
            for p in range(0, self.disc_m):
                if self.discretized_grid[q][p] == 1:
                    self.SanitizedArea.cells.append(Point(x=p*0.2 + self.SanitizedMap.info.origin.position.y + 0.15/2, y=q*0.2 + self.SanitizedMap.info.origin.position.x+ 0.15/2, z=0))

        # load discretized walls positions in self.WallsCells.cells (walls area rappresented with "10" in self.discretized_grid)
        for q in range(0, self.disc_n):
            for p in range(0, self.disc_m):
                if self.discretized_grid[q][p] == 10:
                    self.WallsCells.cells.append(Point(x=p*0.2 + self.SanitizedMap.info.origin.position.y + 0.15/2, y=q*0.2 + self.SanitizedMap.info.origin.position.x+ 0.15/2, z=0))
        ...
        for q in range(0, self.disc_n):
            for p in range(0, self.disc_m):
                if self.discretized_grid[q][p] == -1:
                    print("\nim here")
                    self.UnexploredCells.cells.append(Point(x=p*0.2 + self.SanitizedMap.info.origin.position.y + 0.15/2, y=q*0.2 + self.SanitizedMap.info.origin.position.x+ 0.15/2, z=0))
    ...
    def compute_angles(self, xx_pose,yy_pose,i,j):# <------------------------------------------------------------------------------------ NOTE what represent those variables exactly ?
        ''' This function was written by Alessandro and it return ??? given tha position of the robot(in which reference system) and
         the cell position in the discretized map
        '''
        verbose=False
        ...
        xx_robot=np.round(xx_pose*cellDimensionInMeters+cellDimensionInMeters/2,decimals=2)# posizione non discretizzata
        yy_robot=np.round(yy_pose*cellDimensionInMeters+cellDimensionInMeters/2,decimals=2)
        xx_cell_left=np.round(xx_robot+(i-xx_pose)*cellDimensionInMeters-0.1,decimals=2)
        xx_cell_right=np.round(xx_robot+(i-xx_pose)*cellDimensionInMeters+0.1,decimals=2)
        yy_cell_top=np.round(yy_robot+(j-yy_pose)*cellDimensionInMeters-0.1,decimals=2)
        ...
        if (j-yy_pose)<0 and (i-xx_pose)>0: # the cell is in 1st quadrant
            ...
            alpha_max = 180-np.rint(np.arctan((yy_robot-yy_cell_bottom) /(xx_robot-xx_cell_left))*fromRadtoDegree)
            alpha_min = 180-np.rint(np.arctan((yy_robot-yy_cell_top) / (xx_robot-xx_cell_right))*fromRadtoDegree)
        elif (j-yy_pose)>0 and (i-xx_pose)<0: # the cell is in 3rd quadrant
            alpha_max = 180+np.rint(np.arctan((yy_cell_bottom-yy_robot)/(xx_robot-xx_cell_right))*fromRadtoDegree)
            alpha_min = 180+np.rint(np.arctan((yy_cell_top-yy_robot)/(xx_robot-xx_cell_left))*fromRadtoDegree)
        elif (j-yy_pose)>0 and (i-xx_pose)>0: # the cell is in 4th quadrant
            alpha_max = 270+np.rint(np.arctan((xx_cell_right-xx_robot)/(yy_cell_top-yy_robot))*fromRadtoDegree)
            alpha_min = 270+np.rint(np.arctan((xx_cell_left-xx_robot)/(yy_cell_bottom-yy_robot))*fromRadtoDegree)
        elif (j-yy_pose)==0 and (i-xx_pose)>0: # the cell is On the right
            alpha_max = 360-np.rint(np.arctan((cellDimensionInMeters/2)/(xx_cell_left-xx_robot))*fromRadtoDegree)
            alpha_min = np.rint(np.arctan((cellDimensionInMeters/2)/(xx_cell_left-xx_robot))*fromRadtoDegree)
        elif (j-yy_pose)<0 and (i-xx_pose)==0: # the cell is On the top
            alpha_max = 90+np.rint(np.arctan((cellDimensionInMeters/2)/(yy_robot-yy_cell_bottom))*fromRadtoDegree)
            alpha_min = 90-np.rint(np.arctan((cellDimensionInMeters/2)/(yy_robot-yy_cell_bottom))*fromRadtoDegree)
        elif (j-yy_pose)==0 and (i-xx_pose)<0: # the cell is On the left
            alpha_max = 180+np.rint(np.arctan((cellDimensionInMeters/2)/(xx_robot-xx_cell_right))*fromRadtoDegree)
            alpha_min = 180-np.rint(np.arctan((cellDimensionInMeters/2)/(xx_robot-xx_cell_right))*fromRadtoDegree)
        elif (j-yy_pose)>0 and (i-xx_pose)==0: # the cell is On the bottom
            alpha_max = 270+np.rint(np.arctan((cellDimensionInMeters/2)/(yy_cell_top-yy_robot))*fromRadtoDegree)
            alpha_min = 270-np.rint(np.arctan((cellDimensionInMeters/2)/(yy_cell_top-yy_robot))*fromRadtoDegree)
        else: #giving the same positions the result is zero
            alpha_max = 0
            alpha_min = 0
        return int(alpha_min), int(alpha_max)
    ...
    def roomLimitCheck(self, xx_pose,yy_pose,radius,room):
        ''' This function is used to understand when the room is finished and the cycle could be stopped. Written by Alessandro '''
        if (xx_pose+radius)<room.shape[1] or (xx_pose-radius)>0 or (yy_pose+radius)<=room.shape[0] or (yy_pose-radius)>=0:
            return True
        else:
            return False
    ...
    def PerformeSanitization(self):
        ...
        This function manages squares that need to be sanitized and which have been sanitized by moving them from
         ...
         with the percentage of total energy they must receive to be considered sanitized.
        '''
        modelState = rospy.ServiceProxy('/gazebo/get_model_state', GetModelState)# use a service to take data from '/gazebo/get_model_state' and save them in an object of type GetModelState
        # RobotContinuousPosX and RobotContinuousPosY are coordinates in real world not discretized (position of the robot respect "map" frame)
        RobotContinuousPosX, RobotContinuousPosY = modelState('turtlebot3', 'ground_plane').pose.position.x, modelState('turtlebot3', 'ground_plane').pose.position.y# get needed data giving name and reference frame
    ...
    #################################################################################################################################################
        ...
        while np.unique(angles).size == 2 and self.roomLimitCheck(xx_pose,yy_pose,radius,room): #Until the unique elements of the array angle is have a size of one
            radius+=1
            for i in range(-radius,radius+1): # the plus one is because the range function skip the last element
                for j in range(-radius,radius+1):
                    # the functions determine whether the cell is inside the room and skip all the cells that was previously computed.
                    ...
                    if sqrt(i*2+j*2)>=radius and self.roomLimitCheck2(xx_pose+i,yy_pose+j,room):
                        ...
                        # you see an obstacle here
                        if room[yy_pose+j,xx_pose+i] == 10:# <--------------------------------------------------------------------------------------------- NOTE here you have to change the value of obstacle
                            # You cannot change the values of angles also for the cells that have your same raidus
                            # for this reason it has been used a backup variable
                            #The plus one is because the operator misses the last element
                            angles_backup[alpha_min:alpha_max+1] =1
                            ...
                            #the plus one and minus one is because if there is a match of the angles, the cell is still completely uncovered
                            #All those exceptions are for
                            if angles[alpha_min+1] == 0 and angles[alpha_max-1] == 0 or angles[alpha_min-1] == 0 and angles[alpha_max+1] == 0:
                                room[yy_pose+j,xx_pose+i] += 0.5/((i*cellDimensionInMeters)*2+(j*cellDimensionInMeters)*2)# <--------------------------------------------------------------------------------------------- NOTE here you have to change the value of Uncovered_area
                                if (room[yy_pose+j,xx_pose+i] >= 1):# check if the energy level is higher than 1 (more than 100%) after dt has occured
                                    room[yy_pose+j,xx_pose+i] = 1# sets the value to 1 (100%) in this way it will be saved inside self.SanitizedArea.cells array
    ...
    #################################################################################################################################################
        ...
        for q in range(0, self.disc_n):
            for p in range(0, self.disc_m):
                if ((self.discretized_grid[q][p] >= 0) and (self.discretized_grid[q][p] < 1)):# select the area to sanityze
                    self.AreaToSanitize.cells.append(Point(x=p*0.2 + self.SanitizedMap.info.origin.position.y + 0.15/2, y=q*0.2 + self.SanitizedMap.info.origin.position.x+ 0.15/2, z=0))
    ...
    def ReshapeMapLorenzo(self):
        ...
        sizey_cut=np.shape(map)[1]# lenth in y direction (second axis) of map matrix
        discret_sizex=int(sizex_cut/discret_unit)# discretized lenth in x direction (first axis) of the new matrix
        discret_sizey=int(sizey_cut/discret_unit)# discretized lenth in y direction (second axis) of the new matrix
        map_02= np.zeros([discret_sizey, discret_sizex])# declaration of the new discretized matrix
        ...
        for i in range(discret_sizey):
       for j in range(discret_sizex):
       map_02[i,j]= np.ceil(np.sum(map[discret_unit*i:discret_unit*i+discret_unit, discret_unit*j:discret_unit*j+discret_unit])/(discret_unit**2))
       if map_02[i,j]>1:
       map_02[i,j]=10
       elif map_02[i,j]==1:
       map_02[i,j]=1
    ...
    def DisplaySanitizedMapData(self):
        '''This function is used to display the data of self.SanitizeMap without displaying the matrix at the end'''
        # it don't print the data in a nice way
        print("\nHeader\n\tstamp : {0}\n\tframe_id : {1}".format(self.SanitizedMap.header.stamp, self.SanitizedMap.header.frame_id))
        print("\nInfo\n\tresolution : {0}\n\theight: {1}\n\twidth : {2}\n\torigin : {3}\n\tmap_load_time {4}".format(self.SanitizedMap.info.resolution,\
                                                                                                                     self.SanitizedMap.info.height,\
                                                                                                                     self.SanitizedMap.info.width,\
                                                                                                                     self.SanitizedMap.info.origin,\
                                                                                                                     self.SanitizedMap.info.map_load_time))
...
if _name_ == '_main_':# this line of code is used to execute the code below if it is run from terminal, if this python file is imported from another python code the part below will not be executed
    try:
        ...
        rospy.init_node("TestNode")# creating the node
        grid = occupancy_grid_class_copy()# creating an object (it will beused for data manipulation)
        grid.pub = rospy.Publisher("sanitizazion_map_publisher", OccupancyGrid, queue_size=10)# publisher that will publish a OccupancyGrid object, it's used to perform different tests (you can delete this publisherbecouse it is not needed for the correct functionality of the node)
        grid.pub2 = rospy.Publisher("points_to_sanitize", GridCells , queue_size=10)# is used to view all the points that need to be sanitized in rviz
        grid.pub3 = rospy.Publisher("sanitized_points", GridCells , queue_size=10)# is used to view all the points that have been sanitized
        grid.pub4 = rospy.Publisher("walls", GridCells, queue_size=10)# is used to view all the points that aproximate the wall position with the discretization
        grid.pub5 = rospy.Publisher("unexplored_cells", GridCells, queue_size=10)# is used to view all the points that werent explored when the map was created
        rate = rospy.Rate(GlobalRateTime)# how many times the message will be published in one second
        rospy.Subscriber("map", OccupancyGrid, grid.callbackFillSanitizazionMap)# read the environment map from topic "/map" and create a copy of it (also creates a discretized version that will be used later)
        while not rospy.is_shutdown():# continue to publish until a shutdown interrupt don't arrives ( CTRL + C )
